...
enum RefereeCmdId
{
    ...
    REFEREE_CMD_ID_GAME_ROBOT_HP                         = 0x0003, // len: 16, 机器人血量数据, 3Hz, 服务器->全体机器人, 常规链路
    ...
    REFEREE_CMD_ID_GAME_ROBOT_STATE                      = 0x0201, // len: 13, 机器人性能体系数据, 10Hz, 主控模块->对应机器人, 常规链路
    ...
    REFEREE_CMD_ID_BULLET_REMAINING                      = 0x0208, // len: 6, 允许发弹量, 10Hz, 服务器->己方英雄/步兵/哨兵/空中机器人, 常规链路
    ...
    REFEREE_CMD_ID_SENTRY_DECISION_SYNC                  = 0x020D, // len: 6, 哨兵自主决策信息同步, 1Hz, 服务器->己方哨兵机器人, 常规链路
...
struct __attribute__((packed)) RefereeRxDataGameStatus
{
    ...
    uint64_t sync_time_stamp;     // UNIX时间，当机器人正确连接到裁判系统的NTP服务器后生效
};
...
struct __attribute__((packed)) RefereeRxDataBuff
{
    ...
    uint8_t defense_buff;       // 机器人防御增益（百分比，值为 50 表示 50%防御增益）
...
{
    uint8_t projectile_type;         // bit1：17mm弹丸, bit2：42mm弹丸
    uint8_t shooter_number;      // 1: 17mm发射机构, 2: 保留位, 3: 42mm发射机构
    uint8_t launching_frequency; // 单位: Hz
    float projectile_speed;         // 单位: m/s
};
...
struct __attribute__((packed)) RefereeRxDataProjectileAllowance
{
    ...
    uint16_t projectile_allowance_fortress; // 堡垒增益点提供的储备 17mm
                                            // 弹丸允许发弹量；该值与机器人是否实际占领堡垒无关
};
...
struct __attribute__((packed)) RefereeRxDataRfidStatus
{
    ...
    uint32_t ally_central : 1;               // bit 1：己方中央高地增益点
    uint32_t opponent_central : 1;           // bit 2：对方中央高地增益点
    uint32_t ally_trapezoid : 1;             // bit 3：己方梯形高地增益点
    uint32_t opponent_trapezoid : 1;         // bit 4：对方梯形高地增益点
    uint32_t ally_flying_slope_f : 1;        // bit 5：己方地形跨越增益点（飞坡前，靠近己方一侧）
    uint32_t ally_flying_slope_b : 1;        // bit 6：己方地形跨越增益点（飞坡后，靠近己方一侧）
    uint32_t opponent_flying_slope_f : 1;    // bit 7：对方地形跨越增益点（飞坡前，靠近对方一侧）
    uint32_t opponent_flying_slope_b : 1;    // bit 8：对方地形跨越增益点（飞坡后，靠近对方一侧）
    uint32_t ally_ch_down : 1;               // bit 9：己方地形跨越增益点（中央高地下方）
    uint32_t ally_ch_up : 1;                 // bit 10：己方地形跨越增益点（中央高地上方）
    uint32_t opponent_ch_down : 1;           // bit 11：对方地形跨越增益点（中央高地下方）
    uint32_t opponent_ch_up : 1;             // bit 12：对方地形跨越增益点（中央高地上方）
    uint32_t ally_road_down : 1;             // bit 13：己方地形跨越增益点（公路下方）
    uint32_t ally_road_up : 1;               // bit 14：己方地形跨越增益点（公路上方）
    uint32_t opponent_road_down : 1;         // bit 15：对方地形跨越增益点（公路下方）
    uint32_t opponent_road_up : 1;           // bit 16：对方地形跨越增益点（公路上方）
    uint32_t ally_fort : 1;                  // bit 17：己方堡垒增益点
    uint32_t ally_outpost : 1;               // bit 18：己方前哨站增益点
    uint32_t ally_supply_no_resource : 1;    // bit 19：己方与资源区不重叠的补给区 / RMUL 补给区
    uint32_t ally_supply_zone : 1;           // bit 20：己方与资源区重叠的补给区
    uint32_t ally_assembly_buff : 1;         // bit 21：己方装配增益点
    uint32_t opponent_assembly_buff : 1;     // bit 22：对方装配增益点
    uint32_t center_point : 1;               // bit 23：中心增益点（仅 RMUL 适用）
    uint32_t opponent_fort : 1;              // bit 24：对方堡垒增益点
    uint32_t opponent_outpost : 1;           // bit 25：对方前哨站增益点
    uint32_t ally_tunnel_road_down : 1;      // bit 26：己方地形跨越增益点（隧道）（靠近己方一侧公路区下方）
    uint32_t ally_tunnel_road_mid : 1;       // bit 27：己方地形跨越增益点（隧道）（靠近己方一侧公路区中间）
    uint32_t ally_tunnel_road_up : 1;        // bit 28：己方地形跨越增益点（隧道）（靠近己方一侧公路区上方）
    uint32_t ally_tunnel_trapezoid_low : 1;  // bit 29：己方地形跨越增益点（隧道）（靠近己方梯形高地较低处）
    uint32_t ally_tunnel_trapezoid_mid : 1;  // bit 30：己方地形跨越增益点（隧道）（靠近己方梯形高地较中间处）
    uint32_t ally_tunnel_trapezoid_high : 1; // bit 31：己方地形跨越增益点（隧道）（靠近己方梯形高地较高处）

    uint8_t opponent_tunnel_road_down : 1;      // bit 0：对方地形跨越增益点（隧道）（靠近对方一侧公路区下方）
    uint8_t opponent_tunnel_road_mid : 1;       // bit 1：对方地形跨越增益点（隧道）（靠近对方一侧公路区中间）
    uint8_t opponent_tunnel_road_up : 1;        // bit 2：对方地形跨越增益点（隧道）（靠近对方一侧公路区上方）
    uint8_t opponent_tunnel_trapezoid_low : 1;  // bit 3：对方地形跨越增益点（隧道）（靠近对方梯形高地较低处）
    uint8_t opponent_tunnel_trapezoid_mid : 1;  // bit 4：对方地形跨越增益点（隧道）（靠近对方梯形高地较中间）
    uint8_t opponent_tunnel_trapezoid_high : 1; // bit 5：对方地形跨越增益点（隧道）（靠近对方梯形高地较高处）
...
struct __attribute__((packed)) RefereeRxDataGroundRobotPosition
{
    ...
    float infantry_3_x; // 己方 3 号步兵机器人位置 x 轴坐标，单位：m
    float infantry_3_y; // 己方 3 号步兵机器人位置 y 轴坐标，单位：m
    float infantry_4_x; // 己方 4 号步兵机器人位置 x 轴坐标，单位：m
    float infantry_4_y; // 己方 4 号步兵机器人位置 y 轴坐标，单位：m
    float reserved_1;   // 保留
    float reserved_2;   // 保留
};
...
struct __attribute__((packed)) RefereeRxDataRadarMarkData
{
    ...
    uint16_t opponent_hero_1_vulnerable : 1;     // bit 0：对方 1号英雄机器人易伤情况
    uint16_t opponent_engineer_2_vulnerable : 1; // bit 1：对方 2号工程机器人易伤情况
    uint16_t opponent_infantry_3_vulnerable : 1; // bit 2：对方 3号步兵机器人易伤情况
    uint16_t opponent_infantry_4_vulnerable : 1; // bit 3：对方 4号步兵机器人易伤情况
    uint16_t opponent_aerial_special : 1;        // bit 4：对方空中机器人特殊标识情况
    uint16_t opponent_sentry_vulnerable : 1;     // bit 5：对方哨兵机器人易伤情况
    uint16_t ally_hero_1_special : 1;            // bit 6：己方 1号英雄机器人特殊标识情况
    uint16_t ally_engineer_2_special : 1;        // bit 7：己方 2号工程机器人特殊标识情况
    uint16_t ally_infantry_3_special : 1;        // bit 8：己方 3号步兵机器人特殊标识情况
    uint16_t ally_infantry_4_special : 1;        // bit 9：己方 4号步兵机器人特殊标识情况
    uint16_t ally_aerial_special : 1;            // bit 10：己方空中机器人特殊标识情况
    uint16_t ally_sentry_special : 1;            // bit 11：己方哨兵机器人特殊标识情况
...
struct __attribute__((packed)) RefereeRxDataSentryInfo
{
    ...
    // bits 0-10：除远程兑换外，哨兵机器人成功兑换的允许发弹量
    uint32_t remote_exchange_fire_count : 4;
    // bits 11-14：哨兵机器人成功远程兑换允许发弹量的次数
    uint32_t remote_exchange_hp_count : 4;
    // bits 15-18 ：哨兵机器人成功远程兑换血量的次数
    ...
    // bit 0：哨兵当前是否处于脱战状态，1 为脱战，0 为在战
...
struct __attribute__((packed)) RefereeTxDataSentryCmd
{
    ...
    uint32_t sentry_mode_cmd : 2;
    // bit 21-22：哨兵修改当前姿态指令，1 为进攻姿态，2 为防御姿态，3 为移动姿态，默认为
    // 3；修改此值即可改变哨兵姿态。
    uint32_t confirm_power_rune_activation : 1;
    // bit 23：哨兵机器人是否确认使能量机关进入正在激活状态，1 为确认。默认为 0。
    uint32_t reserved : 8;
    // bit 24-31：保留
};
...
struct __attribute__((packed)) RefereeRxDataMapCommandData
{
    ...
    uint8_t cmd_keyboard;    // 云台手按下的键盘按键通用键值；无按键按下时为 0
...
struct __attribute__((packed)) RefereeTxDataMapRobotData
{
    ...
    uint16_t opponent_aerial_position_x;     // 对方 6 号空中机器人 x 位置坐标，单位：cm
    uint16_t opponent_aerial_position_y;     // 对方 6 号空中机器人 y 位置坐标，单位：cm
    ...
    uint16_t ally_aerial_position_x;         // 己方 6 号空中机器人 x 位置坐标，单位：cm
    uint16_t ally_aerial_position_y;         // 己方 6 号空中机器人 y 位置坐标，单位：cm
...
{
    uint16_t key_value_1 : 8; // bit 0-7：按键 1 通用键值
    uint16_t key_value_2 : 8; // bit 8-15：按键 2 通用键值；支持 2 键无冲
    uint16_t x_position : 12; // bit 0-11：鼠标 X 轴绝对像素位置，屏幕左上角为 (0,0)
    uint16_t mouse_left : 4;  // bit 12-15：鼠标左键状态，1 为按下，其他值为未按下
    uint16_t y_position : 12; // bit 0-11：鼠标 Y 轴绝对像素位置
    uint16_t mouse_right : 4; // bit 12-15：鼠标右键状态，1 为按下，其他值为未按下
...
struct __attribute__((packed)) RefereeTxDataCustomInfo
{
    ...
    uint8_t user_data[30]; // UTF-16 编码字符，支持中文，发送时注意大小端
};
...
{
    uint8_t data[30]; // 自定义客户端发送给机器人的自定义指令，最大 30 字节
};
